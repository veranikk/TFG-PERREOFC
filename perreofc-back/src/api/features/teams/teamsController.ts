/**
 * Handles HTTP request and response logic for the teams backend feature.
 * It delegates database work to services and keeps route handlers focused on API behavior.
 */

import type { FastifyRequest, FastifyReply } from 'fastify';
import { teamIdParamsSchema, getSquadQuerySchema } from './teamsSchema.js';
import { getTeamSquad } from './teamsServices.js';
...
export async function getTeamSquadHandler(req: FastifyRequest, reply: FastifyReply) {
  const params = teamIdParamsSchema.safeParse(req.params);
  const query = getSquadQuerySchema.safeParse(req.query);
  if (!params.success || !query.success) {
    return reply.code(400).send({ error: 'Parámetros inválidos' });
  }
  const seasonId = query.data.seasonId ?? (await getCurrentSeasonId());
  try {
    return await getTeamSquad(params.data.id, seasonId);
  } catch (err: any) {
