...
import {
  View, Text, ScrollView, TouchableOpacity, Modal, ActivityIndicator, TextInput, Platform,
} from 'react-native';
...
// Roles que admin (no superadmin) puede asignar
...
function ConfirmModal({
  ...
  reasonLabel, reasonValue, onReasonChange, loading,
}: {
  visible: boolean; title: string; message: string; confirmLabel: string;
  onCancel: () => void; onConfirm: () => void; destructive?: boolean;
  reasonLabel?: string; reasonValue?: string; onReasonChange?: (v: string) => void;
  loading?: boolean;
}) {
  const { colors } = useTheme();
  return (
      ...
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 32 }}>
        <View style={{ backgroundColor: colors.card, borderRadius: 20, padding: 24, gap: 16 }}>
          ...
          {reasonLabel && onReasonChange !== undefined && (
            <View style={{ gap: 6 }}>
              <Text style={{ ...typography.label, color: colors.textMuted }}>{reasonLabel}</Text>
              <TextInput
                value={reasonValue}
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            ...
            <Button label={loading ? '...' : confirmLabel} variant={destructive ? 'destructive' : 'primary'} onPress={onConfirm} style={{ flex: 1 }} disabled={loading} />
          </View>
        </View>
      </View>
    </Modal>
...
export default function GestionUsuarioDetailScreen() {
  ...
  // Roles disponibles para asignar según quien gestiona
  const availableRoles = isSuperAdmin ? ALL_ROLES : ADMIN_ALLOWED_ROLES;

  useEffect(() => {
    api.getAdminUser(id).then((u) => { setUser(u); setLoading(false); });
  }, [id]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView style={{ backgroundColor: colors.bg }} contentContainerStyle={{ paddingBottom: 60 }}>
        {/* Avatar card */}
        <View style={{
          alignItems: 'center', gap: 12,
          paddingTop: 20, paddingBottom: 24, paddingHorizontal: 16,
      ...
      <ConfirmModal
        ...
        reasonLabel={!user.banned ? 'MOTIVO DEL BANEO' : undefined}
        reasonValue={!user.banned ? banReason : undefined}
        onReasonChange={!user.banned ? setBanReason : undefined}
        onCancel={() => { setShowBanConfirm(false); setBanReason(''); }}
        onConfirm={async () => {
          setActionLoading(true);
          try {
            await api.banUser(user.id, !user.banned, banReason || undefined);
            haptics.warning();
            setUser((u) => u ? { ...u, banned: !u.banned } : u);
